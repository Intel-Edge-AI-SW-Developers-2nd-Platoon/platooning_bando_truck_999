...
void * ccu_recv_msg(void * arg) {
	...
	while(1) {
		...
		if (!strcmp(pArray[1],"PACU1")) {
			...
			sscanf(pArray[2], "%d", &(pacu_buf[0].dist[pacu_pos[0]]));	
			...
			sscanf(pArray[2], "%d", &(pacu_buf[1].dist[pacu_pos[1]]));	
...
void * timer_msg(void * arg) {
	...
	while(1) {
		...
		if (global_interval >= 100) {
			...
			sprintf(&msg[1][0], "[GUI]%s@%d@%d@%lf@%lf@%lf@%lf@%lf@%lf@%lf@%lf@%lld@%ld\n", 
			...
			sprintf(&msg[1][0], "[CAR_B]%s@DATA@%d@%d@%lf@%lf@%lf@%lf@%lf@%lf@%lf@%lf@%lld\n", &name[1][0],
